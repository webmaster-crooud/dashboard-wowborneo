	...
	return (
		<div className="relative">
			{isLoading ? (
				...
				<motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} type="button" onMouseEnter={() => setMenuAccount(true)} onMouseLeave={() => setMenuAccount(false)} className="flex items-center justify-end text-start gap-3 group">
					<div className="rounded-full relative overflow-hidden group-hover:rotate-[360deg] bg-black flex items-center justify-center w-12 h-12 group-hover:p-1 object-cover object-center transition-all ease-in-out duration-300">
						<Image src={"/assets/Image-not-found.png"} alt="Photo Profile - First Name" width={200} height={200} style={{ width: "100%", height: "auto" }} loading="lazy" className="max-w-12 mx-auto rounded-full" />
					</div>
			...
			<AnimatePresence>
				{menuAccount && (
					<motion.div initial={{ opacity: 0, height: "0" }} animate={{ height: "auto", opacity: 1 }} transition={{ duration: 0.3 }} exit={{ height: 0, opacity: 0 }} onMouseEnter={() => setMenuAccount(true)} onMouseLeave={() => setMenuAccount(false)} className="absolute z-[55] top-0 mt-12 right-0 w-52 bg-gray-50 rounded-3xl shadow-xl overflow-hidden">
						<div className="relative flex flex-col">
							...
							<hr />
							<button type="button" onClick={handleLogout} className="text-sm px-5 py-3 cursor-alias hover:bg-gray-200 transition-all ease-in-out duration-300 font-medium flex items-center justify-start gap-2 text-gray-700">
								{loading === "logout" ? (
									<>
