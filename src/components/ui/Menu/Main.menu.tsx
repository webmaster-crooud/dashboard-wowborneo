...
const LinkMenu: React.FC<MenuInterface> = ({ title, icon, url }) => {
    ...
    return (
        <Link
            href={url}
            className={twMerge(
                `flex items-center border border-transparent rounded-xl py-2 px-3 text-[17px] text-black justify-start gap-3 ${
                    pathName.startsWith(String(url)) && "bg-brown/10 border-brown"
                }`,
                "hover:border-brown hover:bg-brown/10 duration-300 transition-all ease-in-out"
            )}
        >
            {icon} {title}
        </Link>
